...
const Contact = () => {
  const { data, loading: portfolioLoading } = usePortfolioData();
  ...
  const handleSubmit = (e) => {
    ...
    emailjs
      .send(
        ...
        {
          from_name: form.name,
          to_name: `${data?.personal?.name || '{data?.personal?.name || 'Himanshu'}'} Sangwan`,
          from_email: form.email,
          to_email: `sangwan${data?.personal?.name || '{data?.personal?.name || 'himanshu'}'}8443@gmail.com`,
      ...
      >
        ...
        >
          <label className='flex flex-col'>
            <span className='text-white font-medium mb-4'>{data?.personal?.name || 'Your Name'}</span>
            <input
              type='text'
              name='name'
              value={form.name}
